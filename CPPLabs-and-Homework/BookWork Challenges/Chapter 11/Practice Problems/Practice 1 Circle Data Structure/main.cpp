// This program stores data about a circle in a structure.
#include <iostream>
#include <cmath> //pow function in particular 
#include <iomanip>
using namespace std;

// Constant for pi
const double PI = 3.14159;

// Struct declaration
struct Circle
{
    double radius; // A circle's radius 
    double diameter; // Circle Diameter
    double area; // Circle Area
}; // Each structure has to end with a semicolon

int main()
{
    Circle c; // Defines the structure variable

    // Get the circle's diameter
    cout << "Enter the diameter of a circle: ";
    cin >> c.diameter;

    // Calculate the circle radius
    c.radius = c.diameter / 2;

    // Calculate the circle's area 
    c.area = PI * pow(c.radius, 2.0);

    // Display the cricle data.
    cout << fixed << showpoint << setprecision(2);
    cout << "The radius and area of the circle area:\n";
    cout << "Radius: " << c.radius << endl;
    cout << "Area: " << c.area << endl;
    return 0;

}